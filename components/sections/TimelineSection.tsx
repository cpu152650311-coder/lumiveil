      ...
      <section className="bg-white py-20 px-4">
        <div className="max-w-site mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-6">
              {milestones.map((m, i) => (
                <div key={i} className="relative text-center lg:text-left">
                  <div className="w-10 h-10 bg-primary text-white font-outfit font-bold rounded-full flex items-center justify-center text-sm mx-auto lg:mx-0 mb-4 relative z-10">{i + 1}</div>
          ...
          <div className="mt-8 text-center">
            <span className="inline-block bg-primary-light text-primary font-outfit font-semibold text-sm px-4 py-2 rounded-full">
    ...
    <section className="bg-white py-20 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="relative">
          <div className="absolute left-4 sm:left-1/2 sm:-translate-x-px top-0 bottom-0 w-0.5 border-l-2 border-dashed border-primary" />
          <div className="space-y-10">
            {milestones.map((m, i) => (
              <div key={i} className={`relative flex gap-6 sm:gap-0 ${i % 2 === 0 ? 'sm:flex-row' : 'sm:flex-row-reverse'}`}>
                <div className="hidden sm:block sm:w-1/2" />
                <div className="absolute left-2.5 sm:left-1/2 sm:-translate-x-1/2 w-3.5 h-3.5 bg-primary rounded-full border-4 border-primary-light z-10" />
                <div className="ml-10 sm:ml-0 sm:w-1/2 sm:px-10">
                  <span className="font-outfit font-bold text-primary text-sm">{m.year}</span>
